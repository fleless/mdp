...
class AddUpdateMainDeplacementDialog extends StatefulWidget {
  ListWorkload liste;
  bool isAdd;
  WorkLoadModel workLoadModel;

  AddUpdateMainDeplacementDialog(this.liste, this.isAdd, this.workLoadModel);

  @override
  State<StatefulWidget> createState() => _AddUpdateMainDeplacementDialogState();
}

class _AddUpdateMainDeplacementDialogState
    extends State<AddUpdateMainDeplacementDialog> {
    ...
    return ElevatedButton(
      ...
      onPressed: () {
        if (formKey.currentState.validate()) {
          if (widget.isAdd) {
            _redactionBloc.liste_mainsDeplacement.add(WorkLoadModel(
                widget.liste.id,
                widget.liste.name,
                _qteController.text,
            ...
            int position = _redactionBloc.liste_mainsDeplacement
                .indexOf(widget.workLoadModel);
            _redactionBloc.liste_mainsDeplacement[position].quantity =
                _qteController.text;
            _redactionBloc.liste_mainsDeplacement[position].price_ht =
                _priceController.text;
            _redactionBloc.liste_mainsDeplacement[position].comment =
                _commentController.text;
            Modular.to.pop();
            _redactionBloc.notifyChanges.add(0);
          }
